...
function getAverage(value1, value2) {
  ...
  const res = num === Infinity ? 1.7976931348623157e308 : num;
  return res;
}
...
function toNumber(value, def) {
  ...
  const res = Number.isNaN(newValue) ? def : newValue;
  return res;
}
...
function isPowerOfTwo(num) {
  ...
  for (let i = 0; i < 64; i += 1) {
    number[i] = 2 ** i;
    if (number[i] === num || number[i] > num) break;
  }
  if (Number(number.slice(-1)) === num) res = true;
  else if (Number(number.slice(-1)) > num) res = false;
  return res;
}
